...
export function PageRegister() {
    ...
    function handleFormSubmit(e) {
        ...
        fetch('http://localhost:4824/api/register', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',  // is backend
            },
           
            body: JSON.stringify({ email, password }), // gaunam json objekta email ir password
        })
                .then(res => res.json()) // gaunam respose json
                .then(data => {
                    setResponseType(data.type)  // gaunam duomenu objekta
